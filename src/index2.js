 ...
 */
async function fetchUserRepos(user, options = {}) {
  ...
  while (true) {
    ...
    repos.push(...data.map((r) => ({ name: r.name, full_name: r.full_name, html_url: r.html_url })));
 ...
 */
export async function fetchOpenIssuesForAllRepos(user, options = {}) {
 ...
 */
export async function fetchLatestIssues(owner, repo, options = {}) {
  if (!owner || !repo) {
...
(async () => {
  ...
  if (!owner) {
    console.error('Usage: node src/index2.js <owner>');
    console.error('\nExample: node src/index2.js ja1101');
    process.exit(1);
  }
